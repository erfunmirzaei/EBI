    ...
    for i in tqdm(range(configs.n_repits)):    
        ...
        for j in range(len(Ns)):
            ...
            if configs.n_sample_thresh >= n:
                ...
                X = data_points[0:Ns[j]][:,i]
                X = X.reshape(X.shape[0], -1)
                m = n / configs.n_sample_thresh
                assert m % 2 == 0, "m must be even"
                cov_biased = 0
                cov_unbiased = 0
                for l in range(m):
                    for m in range(m):
                        kernel_matrix = gauss_kernel(X[l*configs.n_sample_thresh:(l+1)*configs.n_sample_thresh], X[m*configs.n_sample_thresh:(m+1)*configs.n_sample_thresh])
                        if l == m:
                            cov_biased += biased_covariance_estimator(kernel_matrix, tau= tau)
                        else:
                            cov_unbiased += biased_covariance_estimator(kernel_matrix, tau= tau)

                cov_unbiased = max(cov_biased - (cov_unbiased / (m-1)), 0)/m
                cov_biased = cov_biased / m            
