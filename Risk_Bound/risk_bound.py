...
def risk_bound_N_OU(train_dataset, val_dataset, test_dataset, Ns, lamda, length_scale, configs):
    ...
    for i in tqdm(range(configs.n_repits)):    
        for j in range(len(Ns)):
            ...
            kenel_matrix = kernel_model.kernel_matrix(train_data)
            biased_cov_est = biased_covariance_estimator(kenel_matrix, tau)
            unbiased_cov_est = unbiased_covariance_estimator(kenel_matrix, tau)
            ...
            # TODO: Compute the Variance trace(D- D_hat) 
            diag_elss = np.diagonal(kenel_matrix)
            diag_elss = diag_elss.reshape(2*m, tau)
